...
import CollapsibleSection from "@/components/TutorListingComponents/CollapsableSection";
import Checkbox from "@/components/TutorListingComponents/checkbox";
import Search from "@/components/TutorListingComponents/search";
import Slider from "@/components/TutorListingComponents/slider";
import BasicDateTimePicker from "@/components/TutorListingComponents/DateTimePicker";
import TutorList from "@/components/TutorListingComponents/TutorList";
import dayjs, { Dayjs } from "dayjs";
import BookingModal from "@/components/BookingComponents/BookingModal";
import UpcomingSession from "@/components/TutorListingComponents/UpcomingSession";
...
export default function Page() {
  ...
  const allUniversities = Array.from(
    new Set(tutors.map((tutor) => tutor.institution))
  );
  ...
  const filteredTutors = tutors.filter((tutor) => {
    ...
    const matchesUniversities =
      ...
      selectedUniversities.includes(tutor.institution);
    const matchesPrice =
      !priceRange ||
      ...
      (Array.isArray(tutor.availability) &&
        tutor.availability.some((slot) => {
          ...
          return (
            ...
            selectedHour >= parseInt(slot.startTime.split(":")[0]) &&
            selectedHour < parseInt(slot.endTime.split(":")[0])
          );
        }));
    const matchesName = `${tutor.firstname} ${tutor.lastname}`
      .toLowerCase()
      .includes(tutorNameSearchTerm.toLowerCase());
        ...
        <div className="flex flex-wrap gap-2 mt-4">
          {selectedSubjects.map((subject) => (
            <FilterTag
              ...
              label={subject}
              onRemove={() =>
                toggleValue(selectedSubjects, setSelectedSubjects, subject)
              }
            />
          ))}
          {selectedUniversities.map((uni) => (
            <FilterTag
              ...
              label={uni}
              onRemove={() =>
                toggleValue(selectedUniversities, setSelectedUniversities, uni)
              }
            />
          ))}
          {priceRange && (
          ...
          {selectedDateTime && (
            <FilterTag
              label={`Date: ${dayjs(selectedDateTime).format(
                "YYYY-MM-DD HH:mm"
              )}`}
          ...
          <CollapsibleSection title="Subject">
            ...
            {(isSubjectExpanded
              ...
              <Checkbox
                ...
                setSelected={(val) =>
                  toggleValue(selectedSubjects, setSelectedSubjects, label)
                }
              />
            ))}
      ...
      <div className="w-full h-screen lg:w-[70%] xl:w-[75vw] flex flex-col flex-1 overflow-y-auto scrollbar-hover p-6">
        <div className="flex items-center justify-between">
          ...
          <div className="relative inline-block">
            <Button onClick={() => setShowUpcoming(true)} variant="secondary">
              <div className="inline-flex items-center gap-2 justify-center">
                <HiOutlineCalendarDays className="text-[18px]" />
                <TextMd className="hidden sm:inline-block">
                  Upcoming Session
                </TextMd>
              </div>
            </Button>
