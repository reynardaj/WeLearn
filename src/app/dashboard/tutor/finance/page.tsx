...
export default function Profile() {
  ...
  useEffect(() => {
    ...
    const fetchTotalEarning = async () => {
        ...
        setError(null);
        try {
            const response = await fetch(`/api/tutor-dashboard/earning/total?tutorId=${tutorId}`);
            if (!response.ok) {
    ...
    <div className="h-screen w-full flex bg-[#F0FAF9] items-center">
      ...
      <div className="w-[85%] h-[85%] flex flex-col ">
        <div className="w-[90%] h-[100%] justify-center rounded-2xl shadow-lg flex flex-col items-center bg-white">
          <div className="w-[95%] h-auto pb-4"> {/* Use w-full */}
            <TextLg className="font-semibold">Total Earning</TextLg>
            <TextMd className="text-xl font-bold">
              {isLoading ? 'Loading...' : error ? 'Error' : `Rp. ${totalEarning?.toLocaleString() ?? 0}`}
            </TextMd>
          </div>
          <div className="w-[95%] h-[80%] flex flex-col"> {/* Use flex-grow */}
            <TextLg className="pb-4">Earning Overview</TextLg>
            <div className="w-full overflow-y-auto no-scrollbar"> {/* This div will give height to EarningOverview */}
