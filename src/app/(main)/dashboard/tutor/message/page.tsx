...
const ContactItem: React.FC<ContactProps> = ({ name, lastMessage, selected, profileimg, onClick }) => (
  ...
  >
    ...
    <div className='flex-1 overflow-hidden'>
      ...
      <TextSm className="text-gray-500 truncate">{lastMessage || 'No messages yet'}</TextSm>
    </div>
  </div>
);
...
export default function MessagePage() {
  ...
  const fetchMessages = async () => {
    ...
    try {
      const res = await fetch(`/api/tutor-dashboard/messages?conversationId=${activeConv}`);
  ...
  const handleSend = async () => {
    ...
    try {
        ...
        setMessages(currentMessages => 
            currentMessages.map(m => m.messageID === optimisticMessage.messageID ? newMsg : m)
        );
        fetchContacts();

    } catch (err) {
        console.error('failed to send', err);
        setMessages(currentMessages => currentMessages.filter(m => m.messageID !== optimisticMessage.messageID));
        ...
        <div className="w-[90%] h-full bg-white rounded-2xl shadow-lg  justify-center flex p-4 gap-4">
          ...
          <div className="w-full md:w-[65%] lg:w-[70%] h-full flex flex-col gap-3">
            ...
            <div className="mt-auto bg-white rounded-xl p-2 flex items-center border border-gray-200 shadow-sm">
              <input
                type="text"
                placeholder={activeConv ? "Write a message..." : "Select a conversation to start"}
                value={draft}
